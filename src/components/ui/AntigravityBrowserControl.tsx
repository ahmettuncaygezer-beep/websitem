...
export default function AntigravityBrowserControl() {
    ...
    const getContextActions = () => {
        const baseActions = [
            { icon: isDark ? <Sun size={18} /> : <Moon size={18} />, label: isDark ? t('sc_day_mode') || 'Gündüz Modu' : t('sc_night_mode') || 'Gece Modu', onClick: toggleDarkMode, color: 'text-amber-400' },
            { icon: <Search size={18} />, label: t('sc_smart_search') || 'Akıllı Arama', onClick: () => (window as any).SelisSearch?.open(), color: 'text-purple-400' },
        ];

        // PWA Install Check
        if ((window as any).SelisPWA?.trigger) {
            baseActions.push({ icon: <Cpu size={18} />, label: t('sc_install_app') || 'Uygulamayı Yükle', onClick: () => (window as any).SelisPWA.trigger(), color: 'text-green-400' });
        }

        if (isPlanner) {
            return [
                {
                    icon: <Box size={18} />, label: t('sc_save_draft') || 'Taslağı Kaydet', onClick: () => {
        ...
        if (isProductPage) {
            return [
                { icon: <Zap size={18} />, label: t('sc_find_similar') || 'Benzerini Bul', onClick: () => { }, color: 'text-gold' },
                ...baseActions
            ];
        }

        return [
            { icon: <Box size={18} />, label: t('sc_room_planner') || 'Oda Planlayıcı', href: '/oda-planlayici', color: 'text-blue-400' },
            ...baseActions
        ];
    };
                ...
                {!isOpen ? (
                    ...
                    >
                        ...
                        <span className="text-[11px] font-sans font-bold uppercase tracking-[0.2em] text-white">
                            {t('sc_antigravity') || 'Antigravity'}
                        </span>
                        <div className="flex items-center gap-1.5 ml-2 border-l border-white/10 pl-3">
                            <div className="flex flex-col items-center">
                                <span className="text-[8px] text-white/40 uppercase leading-none mb-0.5">{t('sc_cart') || 'Cart'}</span>
                                <span className="text-[10px] font-bold text-white leading-none">{cartCount}</span>
                            </div>
                            <div className="w-px h-4 bg-white/5" />
                            <div className="flex flex-col items-center">
                                <span className="text-[8px] text-white/40 uppercase leading-none mb-0.5">{t('sc_fav') || 'Fav'}</span>
                    ...
                    >
                        ...
                        <div className="p-4 border-b border-white/5 flex items-center justify-between">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-[0.1em] text-white/60">{t('sc_title') || 'System Control Center'}</span>
                            </div>
                            <button
                        ...
                        <div className="px-5 py-3 bg-black/40 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                <div className="flex flex-col">
                                    <span className="text-[8px] uppercase text-white/40">{t('sc_core_status') || 'Core Status'}</span>
                                ...
                                <div className="flex flex-col">
                                    <span className="text-[8px] uppercase text-white/40">{t('sc_environment') || 'Environment'}</span>
                        ...
                        <div className="p-4 bg-white/[0.02] mt-2 flex items-center justify-center gap-2">
                            ...
                            <span className="text-[9px] font-bold uppercase tracking-[0.3em] text-warm-gray-light/40 italic">
                                {t('sc_built_with') || 'Built with Antigravity'}
                            </span>
                            <div className="h-px w-8 bg-white/5" />
                        </div>
                    </motion.div>
