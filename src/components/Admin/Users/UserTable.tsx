...
function UserCard({ user, isMe, permissions, canManageUsers }: { user: AdminUser; isMe: boolean; permissions: AdminPermission[] | undefined; canManageUsers: boolean }) {
    return (
        <div className="p-4 border-b border-white/[0.04] bg-[#1C1C1E] active:bg-white/[0.02] transition-colors">
            <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-[12px] text-[#636366] font-mono truncate max-w-[180px]">{user.email}</div>
                    </div>
                </div>
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <button
                        disabled={isMe || !canManageUsers}
                        className={`p-2 transition-colors ${isMe || !canManageUsers ? 'opacity-30' : 'text-[#AEAEB2] hover:text-[#FF453A]'}`}
                    >
                        <Trash2 size={16} />
                    </button>
                    <button className="p-2 text-[#AEAEB2]">
...
export function UserTable({ users }: UserTableProps) {
    const { can, email: currentUserEmail } = usePermissions();
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-white/[0.04] bg-white/[0.02]">
                            ...
                            <th className="px-6 py-4 text-[11px] font-bold text-[#636366] uppercase tracking-wider hidden lg:table-cell">İzinler</th>
                            <th className="px-6 py-4 text-[11px] font-bold text-[#636366] uppercase tracking-wider hidden xl:table-cell">Son Giriş</th>
                                ...
                                <tr key={user.id} className="group hover:bg-white/[0.02] transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ...
                                            <button
                                                disabled={isMe || !canManageUsers}
                                                className={`p-2 rounded-lg transition-colors ${isMe || !canManageUsers
                                                    ? 'text-[#636366] cursor-not-allowed opacity-30'
                                                    : 'text-[#AEAEB2] hover:text-[#FF453A] hover:bg-[rgba(255,69,58,0.05)]'
                                                    }`}
                                                title={isMe ? 'Kendi hesabınızı silemezsiniz' : !canManageUsers ? 'Bu işlem için yetkiniz yok' : 'Sil'}
                                            >
                                                <Trash2 size={16} />
                                            </button>
