...
import type { BlogPost, LocalizedString } from './page';
...
function HeroCard({ post }: { post: BlogPost }) {
    ...
    const langKey = (language?.toLowerCase() || 'tr') as keyof LocalizedString;
    const title = post.title[langKey] || post.title.tr;
    const excerpt = post.excerpt[langKey] || post.excerpt.tr;
            ...
            <div className="relative overflow-hidden rounded-none md:rounded-sm aspect-[16/9] md:aspect-[21/9] bg-[#1C1C1E]">
                ...
                <div className="absolute top-5 left-5 flex gap-2">
                    <span className="flex items-center gap-1.5 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-white rounded-full"
                        ...
                        {t('blog_featured') || (language === 'tr' ? 'Öne Çıkan' : 'Featured')}
                    </span>
            ...
            <article className={`h-full flex flex-col overflow-hidden bg-white dark:bg-[#2C2C2E] border border-[#E8E3DC] dark:border-white/8 hover:border-[#C9A96E] hover:shadow-[0_8px_32px_rgba(201,169,110,0.15)] transition-all duration-400 rounded-sm`}>
                ...
                <div className={`relative overflow-hidden bg-[#E8E3DC] ${size === 'large' ? 'aspect-[4/3]' : 'aspect-[3/2]'}`}>
                    ...
                    <button
                        ...
                        aria-label={language === 'tr' ? 'Kaydet' : 'Save'}
                ...
                <div className="flex-1 flex flex-col p-5">
                    ...
                    <div className="flex items-center justify-between pt-4 border-t border-[#F5F0EB] dark:border-white/8">
                        ...
                        <div className="flex items-center gap-1 text-[10px] text-[#aaa]">
                            ...
                            {post.readingMinutes} {t('blog_min_read') || (language === 'tr' ? 'dk okuma' : 'min read')}
                        </div>
                    </div>
                </div>
            </article>
        ...
        <div className="relative overflow-hidden py-12 px-6 md:px-12 my-12"
            ...
            <div className="relative max-w-2xl mx-auto text-center">
                ...
                <h3 className="text-2xl font-bold text-white mb-2"
                    style={{ fontFamily: 'var(--font-playfair), Playfair Display, serif' }}>
                    {t('blog_newsletter_title') || (language === 'tr' ? 'Haftanın Seçkileri' : 'Weekly Highlights')}
                </h3>
                <p className="text-white/60 text-sm mb-6">
                    {t('blog_newsletter_desc') || (language === 'tr' ? 'Her hafta en ilham verici dekorasyon yazıları ve özel fırsatlar e-posta kutunuza gelsin.' : 'Get the most inspiring decoration articles and special offers in your inbox every week.')}
                </p>
                {sent ? (
                    <p className="text-[#C9A96E] font-semibold text-sm">✓ {t('blog_newsletter_success') || (language === 'tr' ? 'Harika! Sizi listeye ekledik.' : 'Great! We have added you to the list.')}</p>
                ) : (
                    <form className="flex flex-col sm:flex-row gap-3 max-w-md mx-auto"
                        onSubmit={(e) => { e.preventDefault(); if (email) setSent(true); }}>
                        <input
                            type="email"
                            placeholder={t('blog_newsletter_placeholder') || (language === 'tr' ? "E-posta adresiniz" : "Your email address")}
                            value={email}
                        ...
                        <button type="submit"
                            ...
                            {t('blog_newsletter_btn') || (language === 'tr' ? 'Abone Ol' : 'Subscribe')}
                        </button>
                    </form>
                )}
            </div>
...
export default function BlogPageClient({ posts, categories }: Props) {
    const { t, language } = useGlobal();
    const [activeCategory, setActiveCategory] = useState('blog_all');
    ...
    const filtered = useMemo(() => {
        ...
        if (activeCategory !== 'blog_all') {
            result = result.filter(p => p.category === activeCategory);
        }
        if (searchQuery.trim()) {
            const q = searchQuery.toLowerCase();
            const langKey = (language as keyof LocalizedString) || 'tr';
            result = result.filter(p =>
                p.title[langKey].toLowerCase().includes(q) ||
                p.excerpt[langKey].toLowerCase().includes(q) ||
    ...
    return (
        <main className="min-h-screen bg-white dark:bg-[#1C1C1E]">
                ...
                <div className="flex items-end justify-between mb-6">
                    <div>
                        ...
                        <h1 className="text-3xl md:text-5xl font-bold text-[#1C1C1E] dark:text-[#F5F0EB]"
                            style={{ fontFamily: 'var(--font-playfair), Playfair Display, serif' }}>
                            {t('blog_title') || (language === 'tr' ? 'İlham & Rehberler' : 'Inspiration & Guides')}
                        </h1>
                        <p className="text-[#888] dark:text-[#AEAEB2] text-sm mt-2">
                            {t('blog_subtitle') || (language === 'tr' ? 'Uzman tavsiyeleri · Dekorasyon fikirleri · Mobilya rehberleri' : 'Expert advice · Decoration ideas · Furniture guides')}
                        </p>
                    </div>
                    ...
                    <button
                        ...
                        aria-label={t('common_search') || (language === 'tr' ? "Ara" : "Search")}
                    >
                    ...
                    {isSearchOpen && (
                        ...
                        >
                            <input
                                autoFocus
                                type="text"
                                placeholder={t('blog_search_placeholder') || (language === 'tr' ? "Yazı veya konu ara..." : "Search for articles or topics...")}
                                value={searchQuery}
            ...
            <div className="max-w-7xl mx-auto px-4 pb-16">
                ...
                {/* ── Hero post ───────────────────────────────────────────── */}
                {featured && activeCategory === 'blog_all' && !searchQuery && (
                    <div className="mb-10">
                    ...
                    >
                        {filtered.length === 0 ? (
                            <div className="py-24 text-center">
                                <p className="text-[#aaa] text-sm">{t('blog_no_results') || (language === 'tr' ? 'Sonuç bulunamadı.' : 'No results found.')}</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {filtered.map((post, i) => (
                                    <motion.div
                                        ...
                                        className={i === 0 && activeCategory === 'blog_all' && !searchQuery ? 'lg:col-span-2 lg:row-span-1' : ''}
                                    >
                                        <BlogCard
                                            post={post}
                                            size={i === 0 && activeCategory === 'blog_all' && !searchQuery ? 'large' : 'normal'}
                                        />
                                    </motion.div>
                                ))}
                            </div>
                ...
                {/* ── Popular tags ────────────────────────────────────────── */}
                <div className="mt-4">
                    <h2 className="text-[11px] uppercase tracking-widest text-[#C9A96E] font-semibold mb-4">{t('blog_popular_topics') || (language === 'tr' ? 'Popüler Konular' : 'Popular Topics')}</h2>
                    <div className="flex flex-wrap gap-2">
                        {(t('blog_popular_tags') || (language === 'tr' ? 'salon,yatak odası,renk,minimalizm,aydınlatma,trend 2026,depolama,balkon,mutfak' : 'living room,bedroom,color,minimalism,lighting,trend 2026,storage,balcony,kitchen')).split(',').map(tag => (
                            <button
                                key={tag}
