...
"[project]/src/app/koleksiyonlar/[slug]/page.tsx [app-client] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
function CollectionPage({ params }) {
    ...
    const collection = __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$lib$2f$constants$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["COLLECTIONS"].find((c)=>c.slug === slug) || {
        name: 'Koleksiyon',
                        ...
                        switch(slug){
                            case '2026-ilkbahar':
                                filteredProducts = data.filter({
                                    "CollectionPage.useEffect.fetchProducts": (p)=>p.isNew
                                }["CollectionPage.useEffect.fetchProducts"]).slice(0, 12);
                                break;
                            case 'minimalist':
                                // Search for minimalist related terms or just slice
                                filteredProducts = data.filter({
                                    "CollectionPage.useEffect.fetchProducts": (p)=>p.description.toLowerCase().includes('minimalist') || p.description.toLowerCase().includes('sade') || p.materials?.includes('Metal')
                                }["CollectionPage.useEffect.fetchProducts"]).slice(0, 10);
                                if (filteredProducts.length === 0) filteredProducts = data.slice(5, 15);
                                break;
                            case 'klasik':
                                // Search for classic related
                                filteredProducts = data.filter({
                                    "CollectionPage.useEffect.fetchProducts": (p)=>p.description.toLowerCase().includes('klasik') || p.description.toLowerCase().includes('kadife') || p.materials?.includes('Ceviz')
                                }["CollectionPage.useEffect.fetchProducts"]).slice(0, 10);
...
_s(CollectionPage, "c64R4ZorGKGEiEl0lmSPEP/lGO4=", false, function() {
    return [
        __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$context$2f$GlobalContext$2e$tsx__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useGlobal"]
    ];
});
...
//# sourceMappingURL=src_ab3b95e4._.js.map