...
export function SavePlanModal({ open, onClose }: Props) {
    ...
    const handleSave = () => {
        ...
        const plan: SavedPlan = { name, date: new Date().toLocaleString('tr-TR'), state: { room: state.room, furniture: state.furniture, planName: name } as any };
    ...
    const handleLoad = (plan: SavedPlan) => {
        if (plan.state) {
            usePlannerStore.getState().loadPlan({ room: plan.state.room, furniture: (plan.state as any).furniture ?? [], planName: plan.name });
        }
        onClose();
    };
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                ...
                <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} className="w-full max-w-md mx-4 rounded-sm" style={{ background: 'white' }}>
                    <div className="flex items-center justify-between px-6 py-4" style={{ borderBottom: '1px solid #F0EDE8' }}>
                        <h2 className="text-[16px] font-semibold" style={{ color: '#1C1C1E' }}>Planı Kaydet</h2>
                        <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#999' }}><X size={18} /></button>
                    </div>

                    <div className="p-6 space-y-4">
                        <div>
                            <label className="text-[12px] font-medium block mb-1" style={{ color: '#999' }}>Plan Adı</label>
                        ...
                        {saved.length > 0 && (
                            <div>
                                <p className="text-[11px] font-medium uppercase tracking-wider mb-2" style={{ color: '#999' }}>Kayıtlı Planlar</p>
                                        ...
                                        <div key={i} className="flex items-center justify-between p-2 rounded-sm" style={{ background: '#FAFAF8' }}>
                                            ...
                                            <div className="flex gap-2">
                                                <button onClick={() => handleLoad(p)} className="px-2 py-1 text-[11px] font-medium rounded-sm"
                                                    style={{ background: '#C9A96E', color: '#1C1C1E', border: 'none', cursor: 'pointer' }}>Yükle</button>
